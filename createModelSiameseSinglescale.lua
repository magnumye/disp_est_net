...
function createModelSiameseSinglescale(h, w)
    local height, width
    height = h
    width = w
    ...
    local function createAutoEncoder()
        ...
        -- -> size/8createModel_Siamese_disp_smooth_skip
    ...
    local disp_R = disp_net_R(input_imgs_R)
    local norm_disp_L = nn.MulConstant(-1)(disp_L)
    local norm_disp_R = disp_R
    ...
    local disp_grid_R = nn.ReverseXYOrder()(nn.Disparity1DBHWD(height,width)(nn.Transpose({ 2, 3 }, { 3, 4 })(norm_disp_R)))

    -- transpose RGB images (left of stereo) to BHWD
    local tranpos_l_net = nn.Transpose({ 2, 3 }, { 3, 4 })(nn.Identity()(input_imgs_L))
    ...
    local concat_R = {tranpos_l_net, disp_grid_R}

    local disp_L_trans = nn.Transpose({ 2, 3 }, { 3, 4 })(disp_L)
    local disp_R_trans = nn.Transpose({ 2, 3 }, { 3, 4 })(disp_R)
    local concat_disp_L= {disp_R_trans, disp_grid_L}
    local concat_disp_R= {disp_L_trans, disp_grid_R}
    ...
    local output_R = nn.Transpose({ 3, 4 }, { 2, 3 })(nn.BilinearSamplerBHWD()(concat_R)) -- Back to BDHW
    ...
    local input_imgs_R_gray = nn.View(-1,1,height,width)(nn.Mean(2)(input_imgs_R))
    local smoothness_R = nn.SpatialSmoothTerm()({disp_R, input_imgs_R_gray})

    local output_disp_L= nn.Transpose({ 3, 4 }, { 2, 3 })(nn.BilinearSamplerBHWD()(concat_disp_L)) -- Back to BDHW
    local output_disp_R= nn.Transpose({ 3, 4 }, { 2, 3 })(nn.BilinearSamplerBHWD()(concat_disp_R)) -- Back to BDHW
    local diff_disp_L = nn.CSubTable()({output_disp_L, disp_L})
    local diff_disp_R = nn.CSubTable()({output_disp_R, disp_R})
    local disp_est_net = nn.gModule({ input_imgs_L, input_imgs_R }, { output_L, output_R, diff_disp_L, diff_disp_R, smoothness_L, smoothness_R })
